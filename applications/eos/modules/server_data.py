...
def process_data(ftp, station_codes, count, lastest_files_dict, data_folder, station_indicator_dict,
    ...
    for fname in files:
        ...
        current_fm = ''
        try:
            idx = basename.index('_' + station_code + '_')
        except:
            idx = -1
        if idx == -1:
            if file_mapping2:
                try:
                    idx = basename.index('_' + file_mapping2 + '_')
                    current_fm = file_mapping2
                except:
                    idx = -1
        if idx == -1:
            # Todo : ghi ra log nhung tram ko co file du lieu
            logger.info('File is not found station code : %s' % station_code)
            logger.info('File is not found station : %s' % basename)
            continue
        ...
        logger.info('Processing file 1 : %s' % basename)
        count += 1
        # r = ftpReader()
        r = StringIO()
        ...
        try:
            ...
            logger.error('Error server_data {}'.format(e.message))
            res = 1
        # lines = r.data
        lines = ''
        if r.len >0:
        ...
        for row in stations_off:
            if row.station_id == station_id:
                ...
                diff = datetime.now() - row.start_off
                diff = diff.days * 24 * 3600 + diff.seconds

                row.update_record(
                    end_off=datetime.now(),
                    duration=diff
                )
        ...
        if flag_email and False:
            ...
            message = content
            try:
                common.send_mail(mail_to=lst_email, subject=subject, message=message)
