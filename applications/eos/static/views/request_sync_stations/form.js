...
$(document).ready(function () {
  ...
  var recordId = $("#hfRequestSyncStationId").val();
  if (recordId) {
  ...
  $("body").on("change", "#cbbQCVN_8", function (e) {
    ...
    var url = $(this).data("url");
    var data = { qcvn_id: qcvn_id, station_id: $("#hfRequestSyncStationId").val() };
  ...
  $("body").on("click", "#btnLinkIndicatorToWaterStation", function (e) {
    ...
    var data = {
      ...
      station_id: $("#hfRequestSyncStationId").val(),
  ...
  $("body").on("click", "#btnFTP_Connect", function (e) {
    ...
    //   server = encodeURI($("#stations_data_server_public").val());
    // } else {
    //   server = encodeURI($("#stations_data_server").val());
    // }
    // var data = {
    //   data_server: server,
    //   data_folder: $("#stations_data_folder").val(),
    ...
    //
    // if (
    //   (($("#stations_data_server").val() != "") |
    //     ($("#stations_data_server_public").val() != "")) &
    //   ($("#stations_data_folder").val() != "") &
    ...
    //   var folder = encodeURI($("#stations_data_folder").val());
    ...
    //   var url = $(this).data("url");
    //
    //   window.open(url_link, "popup", param);
    //   app.postAjax({
    //     url: url,
    //     data: data,
    //     callback: function (res) {
  ...
  $("#form_preview :input").prop("disabled", true);
...
function loadDataTableForPage_Indicator() {
  ...
  loadDataTable({
    ...
    fnCustomServerData: function (sSource, aoData, fnCallback) {
      aoData.push({
        name: "request_sync_station_id",
        value: $("#hfRequestSyncStationId").val(),
      });
    },
    iTable: 1,
  });
