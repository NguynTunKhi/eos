...
//            },
    ...
    $('#btnSave').click(function () {
        ...
        if (toDate>maxDate)  {
            app.showError(app.translate('ERR_Day_Input_Station_Last_Time_Max_Exceed'));
            return false;
        }
        if (!validateForm()) {
    ...
    $('body').on('click', '#btn-mqtt-check', function (e) {
        check_connect_mqtt({data: { client_id: $('input#stations_mqtt_client_id').val() }, url: $(this).attr('data-url')})
    });

    $('body').on('change', '#cbbIndicatorId', function (e) {
        var indicator_id = $(this).val();
        var qcvn_kind_id = $("#cbbQCVN_TYPE_CODE_AJAX").val();
  ...
  $("body").on("change", "#cbbQCVN_8", function (e) {
    ...
    $('body').on('change', '#cbbQCVN_TYPE_CODE', function (e) {
        ...
        app.postAjax({
            ...
            callback: function (res) {
                if (res.success) {
                    $('#qcvn_detail_min_value').val(Math.ceil(res.qcvn_detail.qcvn_min_value * 1.1 * 1.2));
                    $('#qcvn_detail_max_value').val(Math.ceil(res.qcvn_detail.qcvn_max_value * 1.1 * 1.2));
                    $('#qcvn_detail_const_area_value_1').val(res.qcvn_detail.qcvn_const_area_value);
                    $('#qcvn_detail_const_area_value_2').val(res.qcvn_detail.qcvn_const_area_value);
                    $('#qcvn_detail_const_area_value_3').val(res.qcvn_detail.qcvn_const_area_value);
                } else {
                    app.showError(res.message);
                }
            }
    ...
    $('body').on('click', '#btnLinkIndicatorToWaterStation', function (e) {
        ...
        var data = {
            ...
            qcvn_detail_const_area_value_1: $('#qcvn_detail_const_area_value_1').val(),
            qcvn_detail_const_area_value_2: $('#qcvn_detail_const_area_value_2').val(),
            qcvn_detail_const_area_value_3: $('#qcvn_detail_const_area_value_3').val(),
    ...
    $('body').on('click', '#btnLinkIndicatorToWaterStation_8', function (e) {
        ...
        var length_cbbQCVN_TYPE_CODE_8 = $('#cbbQCVN_TYPE_CODE_8').children('option').length;
        if ((length_cbbQCVN_TYPE_CODE_8 > 1) && ($('#cbbQCVN_TYPE_CODE_8').val() == '') && (temp == false)) {
            $("#cbbQCVN_TYPE_CODE_8").formError(app.translate('LBL_INPUT_MANDATORY_FIELD'));
    ...
    $('body').on('click', '#btnUpdateListLinkStationIndicator1', function (e) {
    var station_type = $(this).data('type');
    var data = {
        station_id: $('#hfRequestCreateStationId').val(),
        station_name: $("#hfStationName").val(),
        convert_rate: $("#convert_rate").val(),
        station_type: station_type,
    };
    ...
    $('body').on('click', '#btnAddStationAlarm', function (e) {
        ...
        var data = {
            ...
            exceed_emails_header: $('#exceed_emails_header').val(),
            ...
            exceed_method_notification: $('#exceed_method_notification').prop('checked'),
    ...
    $('body').on('click', '#btnSaveDatalogger', function (e) {
        ...
        if (($('#logger_id_show').val() != '') & ($('#logger_name_show').val() != '') & ($('#cbbTypeLogger').val() != '')) {
            var url = $(this).data('url');
            app.postAjax({
                url: url,
                data: data,
...
//            app.showError(app.translate('LBL_INPUT_MANDATORY_FIELD'));
    ...
    $('body').on('click', '#btnCreateDatalogger', function (e) {
        ...
        if (($('#logger_id').val() != '') & ($('#logger_name').val() != '') & ($('#cbbTypeLogger').val() != '')) {
            var url = $(this).data('url');
            app.postAjax({
                url: url,
                data: data,
    ...
    try {
        if ($('#send_data_from_date_show').val() != '' && (typeof ($('#send_data_from_date_show').val()) != "undefined")) {
            $('#send_data_from_date_show').val($('#send_data_from_date_show').val().split('T')[0].split('-').join('/') + ' ' + $('#send_data_from_date_show').val().split('T')[1].split('.')[0].split(':').slice(0, 2).join(':'));
        }
    } catch (err) {
    ...
    $('body').on('click', '#btnSaveSendData', function (e) {
        if (($('#send_data_from_date_show').val() == '')) {
            $("#send_data_from_date_show").formError(app.translate('LBL_INPUT_MANDATORY_FIELD'));
        ...
        var data = {
            ...
            from_date: new Date(new Date($('#send_data_from_date_show').val()) - (new Date().getTimezoneOffset() * 60000)).toISOString(),
        ...
        if (($('#time_send_data_show').val() != '')
            & ($('#ftp_ip_show').val() != '') & ($('#ftp_path_show').val() != '') & ($('#ftp_port_show').val() != '')
            & ($('#user_ftp_show').val() != '') & ($('#password_show').val() != '') & ($('#status_show').val() != '')) {
            var url = $(this).data('url');
            app.postAjax({
                url: url,
                data: data,
    ...
    $('body').on('click', '#btnCreatSendData', function (e) {
        ...
        if ($('#send_data_from_date').val() == '') {
            $("#send_data_from_date").formError(app.translate('LBL_INPUT_MANDATORY_FIELD'));
        ...
        var data = {
            ...
            from_date: new Date(new Date($('#send_data_from_date').val()) - (new Date().getTimezoneOffset() * 60000)).toISOString(),
            file_format: $("input[name='repeatMode']:checked").val(),
            file_name: $('#file_name').val(),
        ...
        if (($('#time_send_data').val() != '')
            & ($('#ftp_ip').val() != '') & ($('#ftp_path').val() != '') & ($('#ftp_port').val() != '')
            & ($('#user_ftp').val() != '') & ($('#password').val() != '') & ($('#status').val() != '')) {
            var url = $(this).data('url');
            app.postAjax({
                url: url,
                data: data,
    ...
    $('body').on('click', '#btncheckFTP_SHOW', function (e) {
        ...
        if (($('#ftp_ip_show').val() != '') & ($('#ftp_path_show').val() != '') & ($('#ftp_port_show').val() != '')
            & ($('#user_ftp_show').val() != '') & ($('#password_show').val() != '')) {
            var url = $(this).data('url');
            app.postAjax({
                url: url,
                data: data,
...
//
    ...
    $('body').on('click', '#btncheckFTP', function (e) {
        ...
        if (($('#ftp_ip').val() != '') & ($('#ftp_path').val() != '') & ($('#ftp_port').val() != '')
            & ($('#user_ftp').val() != '') & ($('#password').val() != '')) {
            var url = $(this).data('url');
            app.postAjax({
                url: url,
                data: data,
    ...
    $('body').on('click', '#btncheckFTP_station', function (e) {
        ...
        if (($('#stations_ftp_path').val() != '') & ($('#stations_ftp_user').val() != '') & ($('#stations_ftp_pwd').val() != '')
            & ($('#stations_ftp_ip').val() != '') & ($('#stations_ftp_port').val() != '')) {
            var url = $(this).data('url');
            app.postAjax({
                url: url,
                data: data,
    ...
    $('body').on('click', '#btnAddDataCommand', function (e) {
        ...
        var fields = ['station_name', 'logger_id', 'logger_name', 'type_logger', 'command_name'];
        if (!logger_type) {
...
//                        loadDataTableForPage_DataCommand()
    ...
    $('body').on('click', '#custom_datatable_auto_adjust tbody tr td [name="continous_equal"]', function (e) {
    ...
    $('body').on('click', '#custom_datatable_auto_adjust tbody tr td [name="remove_with_indicator_check"]', function (e) {
        if ($(this).prop('checked') == true) {
            $(this).siblings().attr('readonly', false);
        } else {
            $(this).siblings().attr('readonly', true);
    ...
    $('body').on('click', '#btnAutoAdjust', function (e) {
        ...
        for (i = 0; i < rows.length; i++) {
            ...
            var remove_with_indicator_check = tr.find("[name='remove_with_indicator_check']");
            ...
            if (remove_with_indicator_check.length > 0) {
                param['remove_with_indicator_check'] = remove_with_indicator_check.first().prop("checked");
            }
...
function cbb_logger_type_change() {
    ...
    if (logger_type === 'D_LOGGER') {
        ...
        if (logger_type === 'ADAM') {$('#d-usr-req').html('')} else { $('#d-usr-req').html('*'); }
...
function switch_transfer_type() {
    ...
    $('#m-ftp, #m-mqtt, #btnFTP_Connect, #btncheckFTP_SHOW2, #btn-mqtt-check, .m-client-id').hide()
