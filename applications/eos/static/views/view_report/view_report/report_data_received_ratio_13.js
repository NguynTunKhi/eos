...
$(document).ready(function () {
    $('#cbbAddedStations_chosen').attr('style', 'width: 100%; margin-bottom: 5px; margin-top: 5px;')
        var this_url = $("#cbbStationType").data('url');
        app.postAjax({
            url: this_url,
            ...
            callback: function (res) {
                if (res.success) {
                    $('#cbbAddedStations').html(res.html);
    ...
    $('#cbbStationType').change(function () {
        ...
        app.postAjax({
            ...
            callback: function (res) {
                if (res.success) {
                    $("#check_station").attr("checked", true)
...
function validateTime2() {
    ...
    minDate.setDate(minDate.getDate()-3*365);
     if (fromDate < minDate) {
         app.showError(app.translate('ERR_Day_Input_From_Date_1_MONTH'));
         return false;
     }
    return true;
}
