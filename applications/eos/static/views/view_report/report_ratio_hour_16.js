...
$(document).ready(function () {
    ...
    $('#btnExportExcel').on('click', function (e) {
        ...
        var url = $(this).attr("data-url");
            ...
            var params = {
                'added_columns': $("#cbbAddedColumns").val().toString()
            };
    ...
    $('body').on('click', '.btnCustomSearch', function (e) {
        ...
        if (!$('#Month').val() && $('#Year').val() || !$('#Year').val() && $('#Month').val()) {
            e.preventDefault();
            app.showError(app.translate('Please select both year and month/quarter!'));
            return false;
        }
        if (!validateTime()) {
            e.preventDefault();
    ...
    $('#Month,#Year').change(function () {
        ...
        $('#dt0_from_date').attr('disabled', true);
        $('#dt0_to_date').attr('disabled', true);
        }
        else{
        $('#dt0_from_date').attr('disabled', false);
        $('#dt0_to_date').attr('disabled', false);
        }
    });
