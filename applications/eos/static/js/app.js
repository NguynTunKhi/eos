...
var CommonApp = jQuery.Class({
    ...
    showNotificationWithCallback: function (data) {
        var thisInstance = this;
        var defaults = {
            callback: function () {
                location.reload();
        ...
        $.confirm({
            ...
            buttons: {
                okay: {
                    ...
                    btnClass: 'btn btn-primary',
                    action: function () {
                        $('.nav-tabs li a[href="#tab-8"]').trigger('click');
                        $('.nav-tabs li a[href="#tab-8"]').focus();
                        // location.reload();
                        // data.callback();
                    }
                }
    ...
    postAjax: function (options) {
        ...
        var defaults = {
            ...
            handleResponse: function (data) {
                ...
                if (data.success == false) {
                    thisInstance.showError(data.message);
                    return true;
                }
            }
        };
        ...
        $.ajax({
            ...
            success: function (data) {
                options.handleResponse(data);
                options.callback(data);
            },
            error: function (err) {
                thisInstance.hideProgress();
                thisInstance.showError(err.status + ": " + thisInstance.translate(err.statusText));
            }
        });
    },
    ...
    executeFunctionCustomize: function (instance) {
        var thisInstance = this;
        if ($(instance).attr("disabled")) {
            return true;
        }
...
//        }
        ...
        subExecuteFunction = function (instance, thisInstance) {
            ...
            thisInstance.postAjax({
                url: url,
                data: {ids: ids, station_type: station_type, province_id: province_id},
                callback: function (data) {
                    if (data.success) {
    ...
    overwriteValidator: function () {
        ...
        jQuery.validator.addMethod("greaterThan", function (value, element, params) {
            var value1 = $(element.closest('form')).find('[name="' + params + '"]').val();
            if (!value1) {
                value1 = $(params).val();
            }
            if (!value || !value1) {
            ...
            if (!/Invalid|NaN/.test(new Date(value))) {
                return new Date(value) > new Date(value1);
            }
            return false;
        }, thisInstance.translate('Must be greater than {0}.'));
        // Greater or equal
        jQuery.validator.addMethod("greaterOrEqual", function (value, element, params) {
            ...
            return false;
        }, thisInstance.translate('Must be greater or equal {0}.'));
    },
    ...
    registerEventsForCommonInput: function () {
        ...
        $('.datetime_iso').datetimepicker({
            format: "Y-m-d H:i"
            // format: thisInstance.isoDateTime
        });
    ...
    getCommonAvariables: function () {
        ...
        this.isoDateTime = $("#hfIsoDateTime").val();
    },
    ...
    validateForm: function (formSelector) {
        ...
        var rules = {};
        var items = $("form#frmMain").find("input, select, textarea");
        for (var i = 0; i < items.length; i++) {
            ...
            if (item.attr('maxlength') !== undefined) {
                rule['maxlength'] = item.attr('maxlength');
            }
            // data type
        ...
        if (!$.isEmptyObject(rules)) {
            validator = $(formSelector).validate({rules: rules});
        }
        return true;
    },
