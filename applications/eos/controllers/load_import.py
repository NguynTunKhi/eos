...
#
#   a=request.vars
#   strMea = request.vars.arrStr
#   added_columns = strMea.split(',')
...
#
#   id_indicators = db(conditions).select(db.station_indicator.indicator_id)
...
def graph_detail():
	try:
		...
		if strMea:
			...
			for id in id_indicators:
				id_indicator_station.append(id.indicator_id)
			query = (db.indicators.id.belongs(id_indicator_station,null=True))
			query &= (db.indicators.indicator.belongs(added_columns))
			indicatorOrigin = db(query).select(db.indicators.id)
			for item in indicatorOrigin:
				indicators_ids.append(item.id)
		...
		indicators = db(db.indicators.id.belongs(indicators_ids,null=True)).select(db.indicators.ALL)

		return dict(station_id=station_id, indicators=indicators, show_by=show_by)
	except Exception as ex:
		return dict(success=False, message=str(ex))

# def get_show_by_dict()
...
def get_chart_for_station(*args, **kwargs):
	try:
		...
		try:
			...
			if chart_end:
				chart_end = datetime.strptime(chart_end, '%Y-%m-%d').replace(microsecond=0) + timedelta(hours=23,minutes=59,seconds=59)
		except:
			chart_start = None
			chart_end = None
		# Default
		...
		condition_data = {
			...
			'get_time': {'$gte': show_by_dict[show_by][1], '$lte': show_by_dict[show_by][3]}
		}
		# print condition_data
		attrs = {'get_time': 1, 'data_24h' if table == 'aqi_data_24h' else 'data': 1, '_id': 0}
		rows = pydb[table].find(condition_data, attrs).sort('get_time', 1)
		...
		indicators = db(db.indicators.id.belongs(indicators_id,null=True)).select(db.indicators.indicator, db.indicators.unit)
		...
		# print 'times', (datetime.now() - now).total_seconds()
		...
		for row in rows:
			...
			data_json = row['data_24h' if table == 'aqi_data_24h' else 'data']
			for k in data_json:
				if data.has_key(k):
					try:
		...
		# print 'process data', (datetime.now() - now).total_seconds()
		# now = datetime.now()
		for k in data:
			chart['series'].append({
		...
		# print 'Total', (datetime.now() - total_now).total_seconds()
		# now = datetime.now()
		# dt = list(rows)
		# print 'chart data', (datetime.now() - now).total_seconds()
		return dict(success=True, charts=charts)
	except Exception as ex:
		return dict(success=False, message=str(ex))
...
def indicator_block():
	try:
		...
		conditions = (db.indicators.id.belongs(indicator_ids,null=True))
		...
		for row in rows:
			...
			station_indicator[str(row.id)]['status'] = 1
			station_indicator[str(row.id)]['value'] = ''

		# Lay du lieu cac chi so moi nhat cua station
		conditions = (db.data_lastest.station_id == station_id)
		data_lastest = db(conditions).select(db.data_lastest.data, db.data_lastest.get_time,db.data_lastest.data_status,
											 limitby=(0, 1),
		...
		if data_lastest:
			...
			data_status = data_lastest.data_status
			for k2 in station_indicator:
				_key = str(k2)
				indicator = station_indicator[_key]['name'].decode('utf-8')
				if data_status.has_key(indicator):
					try:
						v = float(data_status[indicator]['value'])
						station_indicator[_key]['value'] = common.convert_data(v)
						station_indicator[_key]['color'] = common.getColorByIndicatorQcvn(si_dict, _key, v)
						station_indicator[_key]['status'] = data_status[indicator]['status']
					except:
						station_indicator[_key]['status'] = 1
...
def indicator_block_old():
	...
	if data_lastest:
		...
		#         except:
		#           break
		for k1 in data_lastest.data_status:
			for k2 in station_indicator:
				if station_indicator[k2]['name'] == k1.encode('utf-8'):
					try:
						v = float(data_lastest.data_status[k1]['value'])
						station_indicator[k2]['value'] = "{0:.2f}".format(v)
						station_indicator[k2]['color'] = common.getColorByIndicatorQcvn(si_dict, k2, v)
						station_indicator[k2]['status'] = data_lastest.data_status[k1]['status']
						break
					except:
						station_indicator[k2]['status'] = 1#data_lastest.data_status[k1]['status']
						break
		#data_status = data_lastest.data_status
		#for k2 in station_indicator:
		#    _key = str(k2)
		...
		#        try:
		#            v = float(data_status[indicator]['value'])
		#            station_indicator[_key]['value'] = common.fix_round(station_indicator[_key]['name'], v)
		#        station_indicator[_key]['color'] = common.getColorByIndicatorQcvn(si_dict, _key, v)
		#            station_indicator[_key]['status'] = data_status[indicator]['status']
		#        except:
		#            station_indicator[_key]['status'] = 1
...
def load_data_for_wbst(*args, **kwargs):
	try:
		...
		list_data = db(conditions).select(orderby=~db.stations.status)
			...
			for item in list_data:
				...
				if item.status == const.STATION_STATUS['OFFLINE']['value']:
					...
					data_lastest = db(conditions_data).select(db.data_lastest.data, db.data_lastest.get_time, limitby=(0, 1),
															  orderby=~db.data_lastest.get_time).first() or None
					if data_lastest:
						if data_lastest.get_time == None:
							if item.off_time == None:
								item.off_time = "-"
								off_text = '(%s)' % (item.off_time) if item.off_time else ''
							else:
								item.off_time = item.off_time
...
def widget_data_collect():
	...
	if data_min_month_this_month:
		...
		if n_qty_this_month:
			n_qty_this_month = common.convert_data(n_qty_this_month)
		# Tinh theo %
		if expected_this_month and expected_this_month > 0:
			...
			if n_this_month:
				n_this_month = common.convert_data(n_this_month)
		else:
			n_this_month = 0
		# n_this_month = "{:,}".format(n_this_month)
	...
	#               'date_month': {'$gte': first_date_in_last_month, '$lt': first_date_in_this_month}}
	# data_min_month_last_month = pydb.data_min_month_collect.find(conditions, {'station_id': 1, 'actual_datamin': 1})
	...
	if data_min_month_last_month:
		...
		if n_qty_last_month:
			n_qty_last_month = common.convert_data(n_qty_last_month)
		if expected_last_month and expected_last_month > 0:
			...
			if n_last_month:
				n_last_month = common.convert_data(n_last_month)
		else:
			n_last_month = 0
	...
	if True:
		...
		conditions = {'station_id': {'$in': station_ids},
					  'date_month': {'$gte': first_date_in_this_month, '$lt': first_date_in_next_month},
					  'number_off_log': {'$gt': 0}}
		rows = pydb.data_min_month_collect.find(conditions, {'_id': 1, 'station_type': 1, 'number_off_log': 1})
	...
	return dict(n_this_month=n_this_month, n_last_month=n_last_month, collect_icon=collect_icon,
				...
				n2_this_month=n2_this_month, n2_last_month=n2_last_month,
				station_type_this_month=station_type_this_month, station_type_last_month=station_type_last_month,
				n_qty_this_month=n_qty_this_month, expected_this_month=common.convert_data(expected_this_month),
				n_qty_last_month=n_qty_last_month, expected_last_month=common.convert_data(expected_last_month),
				)
...
def get_station_distribution_by_province(*args, **kwargs):
	try:
		...
		conditions &= (db.stations.province_id != None)
		rows = db(conditions).select(db.stations.province_id, db.stations.status, limitby=(0, 1000))
		categories = dict()
		if not rows:
		...
		for item in list_data:
			for key in const.STATION_STATUS:
				if const.STATION_STATUS[key]['value'] == item.status:
					...
					break
			off_text = ''
			if item.last_time and status == const.STATION_STATUS['OFFLINE']:
				off_text = '%s %s %s - %s' % (
					T('Offline'), T('From'), item.last_time.strftime(datetime_format_vn), prettydate(item.last_time, T))
			...
			aaData.append([
				A( item.station_name,_href=URL('stations','form', args=[item.id])) + SPAN(off_text, _class='off_text'),
				I(_class=status['icon'], _style='color: %s' % (status['color']), _title=T(status['name'])),
				str(item.id),
				])
...
def station_type():
	...
	for station in stations:
		if station.status == const.STATION_STATUS['EXCEED']['value']:
			if station.station_type == 4:
				total_kk += 1
			elif station.station_type == 1:
				total_nm += 1
			elif station.station_type == 2:
				total_nn += 1
			elif station.station_type == 3:
				total_kt += 1
			elif station.station_type == 0:
				total_nt += 1
