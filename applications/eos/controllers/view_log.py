...
def get_list_log(*args, **kwargs):
    ...
    if duration:  # Todo: Not clear
        ...
        if month and year:
            ...
            if to_date:
                ...
                conditions['get_time']['$lt'] = to_date
        else:
            if from_date:
                if not conditions.has_key('get_time'):
                    conditions['get_time'] = {}
            ...
            if to_date:
                ...
                conditions['get_time']['$lt'] = date_util.string_to_datetime(to_date) + timedelta(days=1)
    ...
    qcvn_dict = dic_station_indicator[station_id] if dic_station_indicator.has_key(
        station_id) else []
    if is_exceed:
            ...
            for indicator in added_columns:
                ...
                if x == 'None' or x == 'NULL' or x == '-' or x.find(',') != - \
                    ...
                    if z or z == 0:
                        ...
                        if qcvn_max and float(x) > qcvn_max:
                            check_qcvn = False
                            title = 'QCVN Max: %s' % qcvn_max
                        if qcvn_min and float(x) < qcvn_min:
                            check_qcvn = False
        ...
        for i, item in enumerate(list_data):
            ...
            for indicator in added_columns:
                ...
                if v == '' or v is None:
                    ...
                    try:
                        if "aqi" in table:
                            val = common.convert_data(round(float(v)))
                        else:
                            val = common.convert_data(float(v))
                        if float(v) > float(max_of_row):
                            max_of_row = float(v)
                    except Exception as s:
                        val = '-'
                row.append(val)
            if "aqi" in table:
                row[2] = common.convert_data(round(float(max_of_row)))
            aaData.append(row)
    ...
    try:
        ...
        for k in indicators:
            ...
            row = [
                ...
                common.convert_data(indicators[k]['max_value']) if indicators[k]['has_max'] else '',
                indicators[k]['max_time'] if indicators[k]['has_max'] else '',
                common.convert_data(indicators[k]['min_value']) if indicators[k]['has_min'] else '',
                indicators[k]['min_time'] if indicators[k]['has_min'] else '',
                common.convert_data(indicators[k]['total'] / indicators[k]['qty']) if indicators[k]['qty'] else '',
            ]
            aaData.append(row)
        return dict(iTotalRecords=iTotalRecords, iTotalDisplayRecords=iTotalRecords, aaData=aaData, success=True)
...
def export_excel():
    ...
    added_columns_decode_with_unit = []

    indicator_rows = db(db.indicators.indicator.belongs(added_columns_decode)).select()
    map_indicator_unit = {}
    for indicator_row in indicator_rows:
        map_indicator_unit[indicator_row.indicator] = indicator_row.unit
    view_type = request.vars.view_type or const.VIEW_BY['MINUTE']['value']
    view_by_info = common.get_const_by_value(const.VIEW_BY, view_type)
    table = view_by_info['table']
    for item in added_columns_decode:
        if item in map_indicator_unit:
            if "aqi" in table:
                added_columns_decode_with_unit.append(item)
            else:
                added_columns_decode_with_unit.append(item + "(" + map_indicator_unit[item] + "}")
    ...
    if list_data is None:
        ...
        if 'aqi' in table:
            ...
            if view_type == 6:
                ...
                vn_aqi_name += 'Ngày'
            temp_headers.append(vn_aqi_name)

        for i, indicator in enumerate(added_columns_decode_with_unit):
        ...
        if is_exceed:
            ...
            for i, item in enumerate(list_data):
                ...
                for indicator in added_columns:
                    ...
                    row.append(val)
                if "aqi" in table:
                    row[2] = common.convert_data(round(max_of_row))
                ws2.append(row)
    # Get station name
...
def export_excel_chart():
    ...
    aaData = []  # Du lieu json se tra ve
    view_by_info = common.get_const_by_value(const.VIEW_BY, view_type)
    table = view_by_info['table']  # value = 2 table = data_hour
...
def get_indicators_and_stations(*args, **kwargs):
    try:
        ...
        data_indicator = db(db.station_indicator.station_id.belongs(stationIds)).select(
