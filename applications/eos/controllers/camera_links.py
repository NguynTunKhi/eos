    ...
    if last_time_access and last_time_refresh:
        ...
        if last_time_access + datetime.timedelta(seconds=access_token_expires - (60 * 60)) > now:
            ...
            if last_time_refresh + datetime.timedelta(seconds=refresh_token_expires - (60 * 15)) > now:
                req = requests.get('{}/zm/api/host/login.json?token={}'.format(iplocal, refresh_token), verify=False,
                                   timeout=5)
                content = req.content
...
def search_camera(*args, **kwargs):
    try:
        ...
        status_camera = dict()
        for idx in dic:
            idx_camera = idx['Monitor_Status']['MonitorId']
            sts_camera = idx['Monitor_Status']['Status']
            status_camera[idx_camera] = sts_camera
        for i in cameras:
            camera_source = str(i.camera_source_zm).replace("http://103.88.113.240" , 'https://{}{}.hap-technology.com'.format("z" , random.randint(1,3)) )
            description = i.description
            id = i.id
            station_name = i.station_name
            camera_id = i.camera_id
            status = ''
            if status_camera.has_key(camera_id):
                status = status_camera[camera_id]
            camera_source = "{}&token={}".format(camera_source, access_token)
            if sts_camera == 'Connected':
                item = " \
                                    <div class='item col-sm-3' style = 'padding:0px'> \
                                        <div class = 'camera' > \
                                            <a> \
                ...
                des = ' ' + description if description else ''
                html += item % (
                    id, camera_source, station_name + des, camera_id, camera_id, T('Camera Record'), camera_id,
                    T('Auto Record'), camera_id, T('Camera History'), camera_id)
            else:
                item = " \
                                                    <div class='item col-sm-3' style = 'padding:0px'> \
                                                        <div class = 'camera' > \
                ...
                des = ' ' + description if description else ''
                html += item % (
                id, '/eos/static/images/video_error.png', station_name + des, camera_id, camera_id, T('Camera Record'),
                camera_id, T('Auto Record'), camera_id, T('Camera History'), camera_id)
            ...
            if not view_only:
                row.append(
                    "%s%s%s%s%s" % (
                        ...
                        I(_class='fa fa-edit'),
                        "</a>"))
                row.append(INPUT(_group='3', _class='select_item', _type='checkbox', _value=item.id))
            row.append(item.id)
...
@service.json
def get_list_camera_by_request_create_station(*args, **kwargs):
    try:
        ...
        conditions = (db.request_create_station_camera_links.request_create_station_id == request_create_station_id)
        list_data = db(conditions).select(db.request_create_station_camera_links.id,
                                          db.request_create_station_camera_links.camera_source,
                                          db.request_create_station_camera_links.station_name,
                                          db.request_create_station_camera_links.camera_id,
                                          db.request_create_station_camera_links.is_visible,
                                          db.request_create_station_camera_links.order_no,
                                          db.request_create_station_camera_links.description)
        iTotalRecords = len(list_data)
        for i, item in enumerate(list_data):
            url = URL('camera_links', 'popup_add_camera_request_create_station', args=[item.id],
                      vars={'request_create_station_id': request_create_station_id, 'type': station_type, 'name': name})
            row = [
                str(i + 1),
                I(_class='fa fa-eye') if item.is_visible else I(_class='fa fa-eye-slash'),
                item.description,
...
@auth.requires(lambda: (auth.has_permission('view', 'camera')))
def popup_add_camera_request_create_station():
    record = db.request_create_station_camera_links(request.args(0)) or None
    request_create_station_id = request.vars.request_create_station_id
    station_type = request.vars.type
    name = request.vars.name

    frm = SQLFORM(db.request_create_station_camera_links, record, _method='POST', hideerror=True, showid=False)
    # frm.custom.widget.description['_rows'] = '2'

    return dict(frm=frm, request_create_station_id=request_create_station_id, type=station_type, name=name)
...
def ajax_save_camera(*args, **kwargs):
    try:
        if not request.vars.is_visible:
            request.vars.is_visible = False
        camera_name = request.vars.description
        station_name = request.vars.station_name
        camera_source = request.vars.camera_source
        id_max = 9999
        if not request.vars.id:
            ...
            iplocal = row['iplocal']
            try:
                list_camera = requests.get('{}/zm/api/{}?token={}'.format(iplocal, 'monitors.json', access_token),
                                           verify=False, timeout=5)
                content = list_camera.content
                dic = json.loads(content)
                try:
                    dic_monitor = dic['monitors']
                    list_camera_id = []
                    for j in dic_monitor:
                        monitorId = j['Monitor_Status']['MonitorId']
                        if monitorId:
                            c = monitorId
                            list_camera_id.append(int(c))
            ...
            camera_name = replace_all(camera_name, d).upper()
            station_name = replace_all(station_name, d).upper()
            payload = {'Monitor[Name]': station_name + '-' + camera_name,
                       'Monitor[Function]': 'Nodect',
                       'Monitor[Path]': camera_source,
                       'Monitor[Width]': '1280',
                       'Monitor[Height]': '720',
                       'Monitor[Type]': 'Ffmpeg',
                       'Monitor[Colours]': '4',
            ...
            source_url = '{host}/zm/cgi-bin/nph-zms?scale=auto&mode=jpeg&maxfps=100&monitor={id}'  # cu
            source_zm = source_url.format(host=url_camera, id=id_max + 1)
            db(db.camera_links.camera_id == 0).update(
                camera_id=id_max + 1,
                camera_source_zm=source_zm
            )
        else:
            data = db(db.camera_links.id == request.vars.id).select(db.camera_links.camera_id,
                                                                    db.camera_links.camera_source)
            db(db.camera_links.id == request.vars.id).update(**dict(request.vars))
            if data[0].camera_source == request.vars.camera_source:
                pass
            else:
                row = db(db.camera_tokens.id > 0).select(db.camera_tokens.ALL).first()
                access_token = None
                if not access_token:
                    access_token = get_token_with()
                payload = {'Monitor[Path]': request.vars.camera_source,
                           'token': access_token}
                url_put = "{host}/zm/api/monitors/{camera_id}.json".format(host=row['iplocal'],
                                                                           camera_id=data[0].camera_id)
                put_api_create_camera = requests.put(url_put, data=payload, verify=False)
...
# @decor.requires_permission('eos|areas|form')
@auth.requires(lambda: (auth.has_permission('create', 'camera') or auth.has_permission('edit', 'camera')))
def ajax_save_camera_create_request_station(*args, **kwargs):
    try:
        if not request.vars.is_visible:
            request.vars.is_visible = False
        camera_name = request.vars.description
        ...
        camera_source = request.vars.camera_source
        if not request.vars.id:
            db.request_create_station_camera_links.insert(**dict(request.vars))
            row = db(db.camera_tokens.id > 0).select(db.camera_tokens.ALL).first()
            access_token = None
            if not access_token:
                access_token = get_token_with()
            ...
            source_url = '{host}/zm/cgi-bin/nph-zms?scale=auto&mode=jpeg&maxfps=100&monitor={id}'  # cu
            source_zm = source_url.format(host=url_camera, id=id_max + 1)
            db(db.request_create_station_camera_links.camera_id == 0).update(
                camera_id=id_max + 1,
                camera_source_zm=source_zm
            )
        else:
            data = db(db.request_create_station_camera_links.id == request.vars.id).select(db.request_create_station_camera_links.camera_id,
                                                                    db.request_create_station_camera_links.camera_source)
            db(db.request_create_station_camera_links.id == request.vars.id).update(**dict(request.vars))
            if data[0].camera_source == request.vars.camera_source:
                pass
            else:
                row = db(db.camera_tokens.id > 0).select(db.camera_tokens.ALL).first()
    ...
    try:
        ...
        for i in cameras:
            ...
            list_camera = requests.get(
                '{}/zm/api/events/index/MonitorId:{}.json?token={}'.format(url, i.camera_id, access_token),
                verify=False, timeout=5)
            content = list_camera.content
            dic = json.loads(content)
            ...
            for i in dic_event:
                ...
                if id_event_camera:
                    id_event_camera_convert = id_event_camera
                ...
                if time_event_camera:
                    time_event_camera_convert = time_event_camera
