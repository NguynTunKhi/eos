...
@service.json
def stationsbycode(*args, **kwargs):
    try:
        station_type = request.vars.station_type
        ...
        conditions = (db.stations.id > 0)
        if station_code:
            conditions &= (db.stations.station_code == station_code)
...
def data(*args, **kwargs):
    try:
        ...
        if station:
            ...
            time_now = last_t - timedelta(days=30)
        conditions = db.data_day_adjust.id > 0
        conditions &= db.data_day_adjust.station_id == station_id
        conditions &= db.data_day_adjust.get_time > time_now
        rows = db(conditions).select(db.data_day_adjust.get_time, db.data_day_adjust.data, orderby=~db.data_day_adjust.get_time,
                                     limitby=(0, 121))
...
def idh_data(*args, **kwargs):
    try:
        ...
        if from_date and to_date:
            ...
            time_end = datetime.now()
            if data_lastest:
                time_end = data_lastest.get_time
            else:
                time_end = datetime(year=time_end.year, month=time_end.month, day=time_end.day, hour=23, minute=59)
            time_start = time_end - timedelta(hours=12)
        ...
        if data_type == '2':
            ...
            time_start = datetime(year=time_end.year, month=time_end.month, day=1, hour=0, minute=0)
            tb = 'data_day_adjust'
        elif data_type == '0':
            tb = 'data_mon'
            time_start = time_end - relativedelta(months=4)
        conditions = db[tb].id > 0
        conditions &= db[tb].station_id == station_id
        conditions &= db[tb].get_time >= time_start
...
def idh_data_lastest(*args, **kwargs):
    try:
        ...
        return dict(success=True, data=indicators_dic.values() , data_lastest=data_lastest)
    except Exception as ex:
        return dict(success=False, message=str(ex))


@service.json
def data_info_stations(*args, **kwargs):
    try:
        stations_id = request.vars.station_id
        data = stations(stations_id)
        return dict(success=True, data=data)
    except Exception as ex:
        return dict(success=False, message=str(ex))
