...
def station():
    ...
    query = db.station_indicator.station_id.belongs(station_ids)
    query &= db.station_indicator.status == 1
    rowsIndicators = db(query).select(db.station_indicator.indicator_id)  # id các indicator
...
################################
def sort_intersection_array(arr_big, arr_small):
    result = filter(lambda x: x in arr_small, arr_big)
    return list(result)
...
def get_list_log_lastest(*args, **kwargs):
    try:
        ...
        if selected_st:
            ...
            rows = db(conditions2).select(db.stations.id, db.stations.status, db.stations.order_no,
        ...
        indicators = db(db.indicators.indicator_type == selected_st).select(db.indicators.id, db.indicators.indicator,
                                                                            db.indicators.unit)
        ...
        for i, item in enumerate(list_data):
            ...
            if added_columns:
                for indicator in indicators:
                    ...
                    if item.data_status:
                        data_status = item.data_status[i_name_decode]['status'] if item.data_status.has_key(
                            i_name_decode) else ''
                    if v or v == 0:
                        try:
                            v = float(v)
                            ...
                            html = SPAN(common.convert_data(v), _style="background:%s; color:white" % c, _class="badge",
                                        _title=title)
                            if (data_status == 2):
                                html += icon
                            if is_qa_qc:
            ...
            if is_qa_qc:
                ...
                row.append(A(I(_class=status_info['icon'],
                               _style=' font-size: 16px !important; color: %s' % (status_info['color']))))
            ...
            row.append(A(I(_class='fa fa-video-camera text-success'),
                         _href=URL('realtime', 'detail_camera', args=[item.station_id])))
...
def approve_data_adjust(*args, **kwargs):
    try:
        ...
        for row in rows:
            ...
            data = {}
            if row.get_time:
                logs.append(row.get_time.strftime("%d-%m-%Y %H:%M"))
...
def get_list_qa_qc(*args, **kwargs):
    try:
        ...
        province_id = request.vars.province_id
        data_type = request.vars.data_type
        min_value = request.vars.min_value
        max_value = request.vars.max_value
        data_filter_by = request.vars.data_filter_by
        is_qa_qc = request.vars.is_qa_qc
        status = request.vars.alarm_level
        s_search = request.vars.sometext
        ...
        if min_value:
            if data_filter_by:
                data_filter_by.append('min_value')
            else:
                data_filter_by = ['min_value']
        if max_value:
            if data_filter_by:
                data_filter_by.append('max_value')
            else:
                data_filter_by = ['max_value']
        if added_columns:
            added_columns = added_columns.split(',')
        ...
        for i, item in enumerate(list_data):
                    ...
                    if not v and v != 0.0:
                        ...
                        try:
                            ...
                            if data_filter_by:
                                is_valid = False
                                for data_filter in data_filter_by:
                                    if data_filter in ['min_value', 'max_value']:
                                        if min_value and max_value:
                                            if v > float(min_value) and v < float(max_value):
                                                is_valid = True
                            ...
                            if data_type == const.DATA_TYPE['YET_APPROVED']['value']:
                                ...
                                if v:
                                    try:
                                        v = float(v)
                                        html += SPAN(common.convert_data(v))
                                    except:
                                        pass
                            added_item[i_name] = html
                        except:
            ...
            if is_qa_qc:
                row.append(
                    INPUT(_type='checkbox', _class='select_item column_item row_all', _row=i, _column=0, _group=0,
                          _value=item.id))
                pass
            else:
        ...
        return dict(iTotalRecords=iTotalRecords, iTotalDisplayRecords=iTotalRecords, aaData=alldata, success=True)
    except Exception as ex:
        return dict(iTotalRecords=0, iTotalDisplayRecords=0, aaData=[], message=str(ex), success=False)
        ...
        try:
            if station_id != '':
                db.manager_stations_history.insert(station_id=station_id,
                                                   action='Approve data',
                                                   username=current_user.fullname or None,
                                                   description='Remove adjust data',
                                                   update_time=datetime.now())
        except Exception as ex:
            pass
        return dict(success=True)
...
def detail():
    ...
    selected_st = int(selected_st)
    selected_st_name = common.get_info_from_const(const.STATION_TYPE, selected_st)[
        'name'] if common.get_info_from_const(const.STATION_TYPE, selected_st) else ''
    indicators = db(db.indicators.indicator_type == selected_st).select(db.indicators.id, db.indicators.indicator,
                                                                        db.indicators.unit)
    items = []
    if station_info:
        ...
        for indicator in indicators:
            ...
            if v:
                try:
                    ...
                    pass
    return dict(station_info=station_info, selected_st=selected_st, station=station, selected_st_name=selected_st_name,
                items=items)
...
def get_list_indicators_by_statios(*args, **kwargs):
    try:
        ...
        if not station_id:
            ...
            stations = db(conditions).select(db.stations.id, db.stations.station_name)
            station_ids = [str(item.id) for item in stations]  # id các trạm
            rowsIndicators = db(db.station_indicator.station_id.belongs(station_ids)).select(
...
def calc_aqi_data_adjust_24h_stations(station_id, time_cal):
    ...
    try:
        ...
        conditions2 &= (db.data_day_adjust.get_time < datetime.datetime.now().replace(hour=0, minute=0, second=0,
                                                                                      microsecond=0))
                ...
                if data['wqi']:
                    ...
                    if time_cal:
                        if item.get_time < time_cal:
                            ...
                            db(db.stations.id == item.station_id).update(qi_adjust=data['wqi'],
                                                                         qi_adjsut_time=item.get_time)
                            db.commit()
                    else:
                        db(db.stations.id == item.station_id).update(qi_adjust=data['wqi'],
                                                                     qi_adjsut_time=item.get_time)
