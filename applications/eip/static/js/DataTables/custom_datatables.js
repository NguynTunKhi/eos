			...
			if (reloadDT[options.iTable]){
				...
				for(var i = 0; i < total; i++){
					if (added_columns[i]){
						...
						var html = '<th class="added_columns">' + columnName + '</th>';
...
$(document).ready(function (e) {
	...
	for (var i=0; i<total; i++){
		let custom_datatable = $(custom_datatables[i]);
    	let sAjaxSource = custom_datatable.attr("data-url");
    	let aoColumns = eval(custom_datatable.attr("data-aoColumns"));
    	let aoClass = eval(custom_datatable.attr("data-aoClass"));
    	let iTable = custom_datatable.attr('data-iTable');
    	let sDom = custom_datatable.attr('data-sDom');
    	let fnCustomDrawCallback = custom_datatable.attr('data-fnCustomDrawCallback');
    	let fnCustomServerData = custom_datatable.attr('data-fnCustomServerData');
		loadDataTable({
			sAjaxSource: sAjaxSource,
			aoColumns: aoColumns,
			aoClass: aoClass,
